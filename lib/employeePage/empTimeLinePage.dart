...
/*
  ...
  Future<TimeLineModal> getTimeLine(String sessionId) async {
    ...
    try {
      String conn = ApiDetails.server;
      String apiUrl = ApiDetails.timeLineApi;
      var urlapi = Uri.parse("$conn$apiUrl?sessionId=$sessionId&empId=$empId&date=$selectedDate");
      ...
      return timeLineModal; // ✅ Always return a valid object
    } catch (e) {
      ...
      // ✅ Instead of returning null, return an empty TimeLineModal object
  ...
  Widget build(BuildContext context) {
    ...
    builder: (context, snapshot) {
      ...
      // ✅ Timeline UI when data is available
      return Timeline.tileBuilder(
        ...
        builder: TimelineTileBuilder.connected(
          ...
          // ✅ Custom Icons Instead of Default Dots
          indicatorBuilder: (context, index) => Indicator.widget(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                color:   getColor(timeLineModalGlobal!.data![index].type), // Background color
              ),
              child: Icon(
          ...
          contentsBuilder: (context, index) {
            ...
            } else if(timeLineModalGlobal!.data![index].type! == "Logged OUT") {
            colorChange = Mythemes.dangerColor;
            } else if(timeLineModalGlobal!.data![index].type! == "Attendance IN") {
              colorChange = Mythemes.successColor;
            } else if(timeLineModalGlobal!.data![index].type! == "Attendance OUT") {
              colorChange = Mythemes.dangerColor;
            }
            else if(timeLineModalGlobal!.data![index].type! == "Work Done") {
  ...
  Color getColor(String? color) {
    switch(color) {
      case "Work Done":
        return Mythemes.lightBluishColor; // ✅ Work Done Icon
      case "Logged IN":
        return Mythemes.successColor; // ✅ Logged In Icon
      case "Logged OUT":
        return Mythemes.dangerColor; // ✅ Logged Out Icon
      case "Break":
        return Mythemes.alertColor; // ✅ Break Icon
      case "Attendance IN":
        return Mythemes.successColor; // ✅ Work Done Icon
      case "Attendance OUT":
        return Mythemes.dangerColor; // ✅ Work Done Icon
      default:
        return Mythemes.lightBluishColor; // ✅ Default Icon (for unknown types)
    }
  }
  ...
  IconData getTimelineIcon(String? type) {
    switch (type) {
      case "Work Done":
        return Icons.task_alt; // ✅ Work Done Icon
      case "Logged IN":
        return Icons.login; // ✅ Logged In Icon
      case "Logged OUT":
        return Icons.logout; // ✅ Logged Out Icon
      case "Break":
        return Icons.free_breakfast; // ✅ Break Icon
      case "Meeting":
        return Icons.business; // ✅ Meeting Icon
      case "Attendance IN":
        return Icons.touch_app; // ✅ Work Done Icon
      case "Attendance OUT":
        return Icons.touch_app; // ✅ Work Done Icon
      default:
        return Icons.circle; // ✅ Default Icon (for unknown types)
    }
  }
}
