...
class ClaimMssApproval extends StatefulWidget {
  ...
  const ClaimMssApproval(
      {Key? key, required this.levelStatus, required this.empId})
      : super(key: key);
  @override
...
class _ClaimMssApprovalState extends State<ClaimMssApproval> {
  ...
  Future getSharedPrfanceList() async {
    ...
    Future<ClaimMssApprovalDataModal> getEmployeeList11 = getEmployeeList(sessionId!);
    final loading = Row(
  ...
  void _runFilter(String enteredKeyword) {
    ...
    if (enteredKeyword.isEmpty) {
      ...
      results = allUsernew?.where((element) =>
          element.reimburName!.toLowerCase().contains(enteredKeyword.toLowerCase())).toList();
  ...
  Widget build(BuildContext context) {
    return DismissKeyboard(
      child: Scaffold(
        ...
        child: Column(
              children: [
              Expanded(child: isLoadingNew
                  ? Center(child: CircularProgressIndicator()) // Show loader
                  : claimMssApprovalDataModalGlobaled == null ?
                Center(child: CircularProgressIndicator()): MyStatelessWidget(claimMssApprovalDataModalGlobaled!)),
              ],
              ),
              )
...
class _MyStatelessWidgetState extends State<MyStatelessWidget> {
  ...
  var stepFour;
  var stepFive;
  void showApprovalDialog(BuildContext context, String action, dynamic claimRaiseId) {
    TextEditingController remarksController = TextEditingController();
  ...
  Future<void> approvedClaim(String remarks, dynamic claimRaiseId, dynamic approveAmt) async {
    String conn = ApiDetails.server;
        ...
        child: RefreshIndicator(
          onRefresh: () {
            Navigator.pushReplacement(
                ...
                PageRouteBuilder(
                  pageBuilder: (a, b, c) =>
                      ClaimMssApproval(levelStatus: levelStatusChecked,empId: empIdReceived,),
                          ...
                          children: [
                            ElevatedButton(
                              onPressed: () {
                                ...
                                print("$claimRaiseId");
                                showApprovalDialog(context, "Disapprove", claimRaiseId);
                                setState(() {});
                              },
                              style: ButtonStyle(
                                backgroundColor: MaterialStateProperty.all(Mythemes.dangerColor),
                              ),
                            ...
                            ElevatedButton(
                              onPressed: () {
                                claimRaiseId = foundDataNew![index].claimId;
                                print("$claimRaiseId");
                                showApprovalDialog(context, "Approve", claimRaiseId);
                                setState(() {});
                              },
                              style: ButtonStyle(
                                backgroundColor: MaterialStateProperty.all(Mythemes.successColor),
                              ),
                              child: Text("Approve"),
                            ).wh(130, 40).py12(),
                          ],
