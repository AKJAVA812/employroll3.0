...
class MarkAttendanceAI extends StatefulWidget {
  final dynamic empId;
  const MarkAttendanceAI(
      {super.key, required this.empId});
  @override
  State<MarkAttendanceAI> createState() => _MarkAttendanceAIState(empId.toString());
}
...
class _MarkAIAttendanceState extends State<MarkAIAttendance> {
  ...
  Future<void> getFaceData(BuildContext context, String coordinates, File image) async {
    ...
    try {
      ...
      apiUrl = (value == 0) ? ApiDetails.faceRecognizeSelf : ApiDetails.faceRecognizeOther;
      print("🔹 Selected API: $apiUrl");
